...
import { Activity, Loader2, Mail, Lock, User, X, Eye, EyeOff, CheckCircle2, ArrowLeft } from 'lucide-react';
...
const Register: React.FC = () => {
  ...
  const getPasswordRequirements = () => [
    { met: password.length >= 8, text: '至少8位' },
          ...
          {step === 'form' ? (
            ...
            <div className="bento-tile p-6">
              <div className="flex items-center justify-between mb-1">
                <h2 className="text-lg font-semibold text-zinc-900">注册账号</h2>
                {localError && (
                  <span className="text-sm text-red-600 bg-red-50 px-3 py-1 rounded-full">{localError}</span>
                )}
              </div>
              <p className="text-xs text-zinc-500 mb-4">请填写以下信息完成注册</p>
              ...
              <form onSubmit={handleFormSubmit} className="space-y-4">
                ...
                <Button
                  type="submit"
                  className="w-full btn-ice h-12 text-base rounded-xl"
                >
                  获取邮箱验证码
                </Button>
              </form>
